...
func (f *firmwareTester) FirmwareUpdateBIOS(ctx context.Context, fileName string) (err error) {
	if f.MakeErrorOut {
		return errors.New("failed update")
	}
	return nil
...
func TestUpdateBMCFirmware(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ctx, cancel := context.WithTimeout(context.Background(), tc.ctxTimeout)
			defer cancel()
			err := UpdateBMCFirmware(ctx, "foo", []BMCFirmwareUpdater{&testImplementation})
			if err != nil {
				diff := cmp.Diff(tc.err.Error(), err.Error())
				if diff != "" {
					t.Fatal(diff)
...
func TestUpdateBMCFirmwareFromInterfaces(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := UpdateBMCFirmwareFromInterfaces(context.Background(), "foo", generic)
			if err != nil {
				diff := cmp.Diff(tc.err.Error(), err.Error())
				if diff != "" {
					t.Fatal(diff)
...
func TestUpdateBIOSFirmware(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ctx, cancel := context.WithTimeout(context.Background(), tc.ctxTimeout)
			defer cancel()
			err := UpdateBIOSFirmware(ctx, "foo", []BIOSFirmwareUpdater{&testImplementation})
			if err != nil {
				diff := cmp.Diff(tc.err.Error(), err.Error())
				if diff != "" {
					t.Fatal(diff)
...
func TestUpdateBIOSFirmwareFromInterfaces(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := UpdateBIOSFirmwareFromInterfaces(context.Background(), "foo", generic)
			if err != nil {
				diff := cmp.Diff(tc.err.Error(), err.Error())
				if diff != "" {
					t.Fatal(diff)
