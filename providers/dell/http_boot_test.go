...
// TestSetHTTPBootTLSMode verifies SetHTTPBootTLSMode PATCHes the HttpDev1TlsMode BIOS Setup
// attribute (device slot 1, see httpBootDeviceIndex).
func TestSetHTTPBootTLSMode(t *testing.T) {
	...
	mux.HandleFunc("/redfish/v1/Systems/System.Embedded.1/Bios", func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPatch:
			...
			body, err := io.ReadAll(r.Body)
			require.NoError(t, err)
			assert.Contains(t, string(body), `"HttpDev1TlsMode":"None"`)
			w.WriteHeader(http.StatusNoContent)
		default:
			w.WriteHeader(http.StatusMethodNotAllowed)
		}
	...
	ok, err := client.SetHTTPBootTLSMode(context.Background(), bmc.HTTPBootTLSModeNone)
	require.NoError(t, err)
	assert.True(t, ok)
	assert.True(t, patched, "expected a PATCH to the Bios resource setting HttpDev1TlsMode")
}
